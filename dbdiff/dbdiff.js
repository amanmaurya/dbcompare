...
class DbDiff {
  ...
  _compareTables (table1, table2) {
    ...
    diff1.forEach((columnName) => {
      var col=table2.columns.find((column) => column.name.toLowerCase() === columnName.toLowerCase())
      if(col){
        simler.push(columnName.toLowerCase())
      // this._safe(`ALTER TABLE ${tableName} change  COLUMN ${this._quote(columnName)} ${this._quote(col)} ${this._columnDescription(col)};`)
      var comment =`-- Previous column Name was ${columnName}\n` ;
      this._safe(`${comment} ALTER TABLE ${tableName} change  COLUMN ${this._quote(columnName)} ${this._quote(col.name)} ${this._columnDescription(col)};`)

      }
    })

    diff1.forEach((columnName) => {
      // console.log(simler,simler.indexOf(columnName.toLowerCase())==-1,columnName,'ssssssssssssssssss')
      if(simler.indexOf(columnName.toLowerCase())==-1)
    ...
    common.forEach((columnName) => {
      ...
      if (this._dialect === 'mysql' && !_.isEqual(col1, col2)) {
        ...
        var comment = col1.type !== col2.type ? `-- Previous data type was ${col1.type}\n` : ''
            comment += col1.nullable !== col2.nullable ? `-- Previous nullable was ${col1.nullable}\n` : ''
            comment += col1.COLLATION_NAME !== col2.COLLATION_NAME ? `-- Previous COLLATION_NAME was ${col1.COLLATION_NAME}\n` : ''
            comment += col1.CHARACTER_SET_NAME !== col2.CHARACTER_SET_NAME ? `-- Previous CHARACTER_SET_NAME was ${col1.CHARACTER_SET_NAME}\n` : ''
  ...
  _columnDescription (col) {
    ...
    // }
    if(col.CHARACTER_SET_NAME != null){
      desc+=' CHARACTER SET '+col.CHARACTER_SET_NAME
    }
    if(col.COLLATION_NAME != null){
    ...
    if(col.COLUMN_DEFAULT ){
      ...
      desc += ' DEFAULT "' + col.default_value+'"'
    }else if(col.nullable=='YES'){
      desc+=' DEFAULT NULL '
      }else if(col.nullable=='NO'){
      desc+=' NOT NULL '
      }

    if(col.EXTRA !=null ){
      desc+=' DEFAULT '+col.EXTRA 
